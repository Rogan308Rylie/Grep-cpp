...
struct PatternComponent {
    ...
    bool has_plus;
    bool has_question;
    int capture_group_id; // 0 for non-capturing, else 1-based
    int backref_id;       // 0 for not a backref, else 1-based
...
bool matches_component(char c, const PatternComponent& component) {
    switch (component.type) {
        case PatternComponent::LITERAL: return c == component.value[0];
        case PatternComponent::DIGIT: return isdigit(c);
        case PatternComponent::WORD: return isalnum(c) || c == '_';
        case PatternComponent::POSITIVE_CLASS:
            for (char pattern_char : component.value) if (c == pattern_char) return true;
            return false;
        case PatternComponent::NEGATIVE_CLASS:
            for (char pattern_char : component.value) if (c == pattern_char) return false;
            return true;
        case PatternComponent::DOT: return true;
        default: return false;
    }
}

int match_component_sequence(const string& input, int pos, const vector<PatternComponent>& components, int comp_idx, MatchState& state) {
    if (comp_idx >= components.size()) return pos;

    const PatternComponent& current = components[comp_idx];
    if (current.type == PatternComponent::BACKREFERENCE) {
        if (state.captures.count(current.backref_id) == 0) return match_component_sequence(input, pos, components, comp_idx + 1, state);
        const string& capture = state.captures[current.backref_id];
        if (pos + capture.length() > input.length()) return -1;
        if (input.substr(pos, capture.length()) != capture) return -1;
    ...
    if (current.type == PatternComponent::ALTERNATION) {
        for (const auto& alternative : current.alternatives) {
            ...
            if (end_alt_match != -1) {
                if (current.capture_group_id > 0) temp_state.captures[current.capture_group_id] = input.substr(start_pos, end_alt_match - start_pos);
                int result = match_component_sequence(input, end_alt_match, components, comp_idx + 1, temp_state);
                if (result != -1) { state = temp_state; return result; }
            }
        }
        return -1;
    }
    ...
    if (current.has_plus) {
        if (!matches_component(input[pos], current)) return -1;
        int max_match_len = 0;
        while (pos + max_match_len < input.length() && matches_component(input[pos + max_match_len], current)) max_match_len++;
        ...
        if (result_skip != -1) { state = temp_state_skip; return result_skip; }
        if (matches_component(input[pos], current)) {
            ...
            if (result_match != -1) { state = temp_state_match; return result_match; }
        }
        return -1;
    } else {
        if (matches_component(input[pos], current)) return match_component_sequence(input, pos + 1, components, comp_idx + 1, state);
        return -1;
    }
}

// --------------
// The parser: always wraps every ( ... ) group as an ALTERNATION PatternComponent with a group ID
vector<PatternComponent> parse_pattern_segment(const string& pattern, int& group_counter) {
    ...
    for (int i = 0; i < pattern.length(); i++) {
        ...
        if (pattern[i] == '^') {
            component = PatternComponent(PatternComponent::START_ANCHOR); component_created = true;
        } else if (pattern[i] == '$') {
            component = PatternComponent(PatternComponent::END_ANCHOR); component_created = true;
        } else if (pattern[i] == '.') {
            component = PatternComponent(PatternComponent::DOT); component_created = true;
        } else if (pattern[i] == '(') {
            int depth = 1, start = i + 1; i++;
            while (i < pattern.length() && depth > 0) {
                if (pattern[i] == '(') depth++; else if (pattern[i] == ')') depth--;
                i++;
            }
            if (depth == 0) {
                string group_content = pattern.substr(start, i - start - 1);
                // Always wrap as ALTERNATION group
                group_counter++; int this_group_id = group_counter;
                PatternComponent alt_component(PatternComponent::ALTERNATION);
                alt_component.capture_group_id = this_group_id;

                // Split group_content by '|' at top-level (not inside nested groups)
                vector<string> alt_strings; string current_alt = ""; int paren_depth = 0;
                for (char c : group_content) {
                    if (c == '(') paren_depth++;
                    else if (c == ')') paren_depth--;
                    else if (c == '|' && paren_depth == 0) { alt_strings.push_back(current_alt); current_alt = ""; continue; }
                    current_alt += c;
                }
                ...
                component = alt_component; component_created = true; i--;
            }
        } else if (pattern[i] == '\\' && i + 1 < pattern.length()) {
            char next = pattern[i + 1];
            if (next == 'd') { component = PatternComponent(PatternComponent::DIGIT); i++; component_created = true; }
            else if (next == 'w') { component = PatternComponent(PatternComponent::WORD); i++; component_created = true; }
            else if (isdigit(next) && (next - '0') >= 1) {
                int backref_id = next - '0';
                component = PatternComponent(PatternComponent::BACKREFERENCE, "", false, false, 0, backref_id); i++; component_created = true;
            } else { component = PatternComponent(PatternComponent::LITERAL, string(1, next)); i++; component_created = true; }
        } else if (pattern[i] == '[') {
            int end = i;
            while (end < pattern.length() && pattern[end] != ']') end++;
            if (end < pattern.length()) {
                string class_content = pattern.substr(i + 1, end - i - 1);
                if (!class_content.empty() && class_content[0] == '^') component = PatternComponent(PatternComponent::NEGATIVE_CLASS, class_content.substr(1));
                else component = PatternComponent(PatternComponent::POSITIVE_CLASS, class_content);
                i = end; component_created = true;
            }
        } else {
            component = PatternComponent(PatternComponent::LITERAL, string(1, pattern[i])); component_created = true;
        }
        if (component_created && i + 1 < pattern.length()) {
            if (pattern[i + 1] == '+') { component.has_plus = true; i++; }
            else if (pattern[i + 1] == '?') { component.has_question = true; i++; }
        }
        if (component_created) components.push_back(component);
    }
    return components;
}
...
bool match_pattern(const string& input, const string& pattern) {
    ...
    if (start_idx < end_idx) effective_components.insert(effective_components.end(), components.begin() + start_idx, components.begin() + end_idx);
    if (effective_components.empty()) {
        if (starts_with_anchor && ends_with_anchor) return input.empty();
        return false;
    }
    int start_search_pos = 0, end_search_pos = input.length();
    if (starts_with_anchor) end_search_pos = 0;
    for (int i = start_search_pos; i <= end_search_pos; ++i) {
        ...
        if (end_match_pos != -1) {
            if (!ends_with_anchor || end_match_pos == input.length()) return true;
        }
    }
    return false;
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3) { cerr << "Expected two arguments" << endl; return 1; }
    string flag = argv[1], pattern = argv[2];
    if (flag != "-E") { cerr << "Expected first argument to be '-E'" << endl; return 1; }
    string input_line; getline(cin, input_line);
    try {
